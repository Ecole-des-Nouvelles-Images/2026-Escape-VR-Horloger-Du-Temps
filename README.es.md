<div align="center">
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/blob/main/README.md">
    <img width="64" height="64" alt="flag_angleterre" src="https://github.com/user-attachments/assets/79223af9-d6d8-48a1-aa17-7355c3c6b444" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/blob/main/README.es.md">
    <img width="64" height="64" alt="flag_espagne" src="https://github.com/user-attachments/assets/22639270-f31b-4416-9445-73230a53ec00" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/blob/main/README.fr.md">
    <img width="64" height="64" alt="flag_france" src="https://github.com/user-attachments/assets/b98ca283-5441-4f57-a475-8cbf1f843485" />
  </a>
</div>

<div align="center">
  <h1>L'Horloger du Temps</h1>
</div>

![IllustrationLoadingScreen](https://github.com/user-attachments/assets/f688900f-af0a-4d44-9c98-7bf49eb5fc59)
[![Unity Version](https://img.shields.io/badge/Unity-2022.3.47-blue?style=flat&logo=unity)](https://unity.com/)
[![GitHub Release](https://img.shields.io/github/release/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps)](https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/releases)
[![License](https://img.shields.io/github/license/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps)](https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/blob/main/LICENSE)
[![Repo Size](https://img.shields.io/github/repo-size/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps?color=lightgrey)](https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps)

**L'Horloger du Temps** es un juego de acertijos en realidad virtual (VR). Un relojero transformó su taller en una **cámara del tiempo** antes de **desaparecer**. Como su **aprendiz**, tendrás que descubrir el **secreto** detrás de su desaparición.

## Índice
1. [Introducción](#introduccion)
2. [Universo](#universo)
3. [Equipo](#equipo)
4. [Jugabilidad](#jugabilidad)
6. [Lanzamiento](#lanzamiento)
7. [Desarrollo](#desarrollo)
8. [Agradecimientos Especiales](#agradecimientos-especiales)
9. [Recomendaciones](#recomendaciones)

---
<div align="center">
  <h1 id="introduccion">Introducción</h1>
</div>

**L'Horloger du Temps** es un juego de acertijos en VR desarrollado durante **cinco semanas**.  
Este proyecto fue creado como parte de un **proyecto académico**.  
El proceso de desarrollo incluyó **una preproducción (1 semana), producción (3 semanas) y una postproducción (1 semana)**.

---
<div align="center">
  <h1>Universo</h1>
</div>

Manipula el flujo del tiempo en un misterioso taller utilizando un **reloj astronómico** para dominar la luz y descubrir los secretos de su maestro desaparecido.

---
<div align="center">
  <h1>Equipo</h1>
</div>

Este juego fue realizado por:

**[Calvin OVIDE](https://github.com/CalvinOvide)** - ***Lead Artist**, Scrum master, Artista 3D, Artista de Entorno, Artista de VFX*  
**[Marion DUBREUIL](https://github.com/Zamelkhia)** - *Artista 3D, Artista de Entorno, Artista de Props*  
**[Maëlys MANICAS](https://github.com/mmanicas)** - *Artista 3D, Artista de Props, Diseñadora de UI*  
**[Hugo ISAMBART](https://github.com/HIsambart)** - ***Lead Developer**, Scrum Master, Integrador*  
**[Julien DURAND](https://github.com/Iluje)** - *Desarrollador de Juego, Integrador*  
**[Alexandre RENAUX](https://github.com/sky-ark)** - *Desarrollador de Juego, Diseñador de Sonido*

---
<div align="center">
  <h1 id="jugabilidad">Jugabilidad</h1>
</div>

*L'Horloger du Temps* es un juego de acertijos en VR en el que el jugador debe **descubrir** el secreto detrás de la **desaparición de su maestro** manipulando el tiempo mientras **resuelve acertijos** dentro de un misterioso taller.

El jugador asume el papel de un aprendiz que se **desplaza físicamente** por un espacio reducido. Un **reloj astronómico** se encuentra en el centro de la habitación, y el objetivo es interactuar con sus agujas para **alterar dinámicamente el tiempo** y el estado físico del entorno. Con cada manipulación del tiempo, los objetos de la habitación **cambian de estado**: las flores se marchitan, la cera de las velas se derrite y los haces de luz se desplazan.

Para avanzar, el jugador debe **observar estos cambios**, analizar las pistas y resolver **tres conjuntos de acertijos interconectados** —basados en flores, cera y luz— para obtener tres llaves únicas.

Si el jugador logra desbloquear la **trampilla final** insertando las tres llaves, **descubrirá la verdad** y heredará la creación definitiva del maestro.

## Mecánicas de juego
El jugador puede **desplazarse**, **agarrar**, **interactuar** y usar **objetos** para resolver el misterio.

https://github.com/user-attachments/assets/c16c3b11-6b20-427c-b284-727f088ed916

---
<div align="center">
  <h1 id="lanzamiento">Lanzamiento</h1>
</div>

El archivo APK del juego está disponible y aquí está el enlace para **[descargarlo](https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/releases)**.

---
<div align="center">
  <h1 id="desarrollo">Desarrollo</h1>
</div>

Este proyecto se encuentra actualmente **en desarrollo**.  

Si encuentras algún error o tienes sugerencias, por favor abre un **[ticket de issue](https://github.com/Ecole-des-Nouvelles-Images/2026-Escape-VR-Horloger-Du-Temps/issues/new)**.

---
<div align="center">
  <h1 id="agradecimientos-especiales">Agradecimientos Especiales</h1>
</div>

Nos gustaría agradecer a nuestra **escuela** y a nuestros **profesores**:  
- **[L'École des Nouvelles Images](https://www.nouvellesimages.xyz/)**
- **[Frédéric BAST](https://github.com/FBast)** - *Soporte de Desarrollo*
- **[Yvan BLADET](https://www.linkedin.com/in/yvan-bladet-676924261/)** - *Soporte de Desarrollo / Integración*
- **[Jerome CROS](https://fr.linkedin.com/in/crospro)** - *Soporte de Arte*
- **[Tommy HA PHUOC](https://fr.linkedin.com/in/tommyhaphuoc/en)** - *Soporte de Jugabilidad*

---
<div align="center">
  <h1>Recomendaciones</h1>
</div>

Para la mejor experiencia, recomendamos jugar en **Meta Quest 3**.
